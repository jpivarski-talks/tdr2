...
my $VERSION = sprintf "%d.%03d", q$Revision: 405 $ =~ /(\d+)/g;
my $verbose;
...
#
   ...
   # \RCS$Revision: 405 $
   # \RCS$Date: 2009-12-15 12:44:28 -0600 (Tue, 15 Dec 2009) $
   # \RCS$HeadURL: svn+ssh://pivarski@svn.cern.ch/reps/tdr2/utils/trunk/general/makeManifest.pl $
   # \RCS$Id: makeManifest.pl 405 2009-12-15 18:44:28Z alverson $
   # -- what it looked like under cvs, with the dollar signs removed
   #\RCS Revision: 1.4 
   #\RCS Date: 2008/07/31 09:20:05 
   #\RCS Name: 
   my $repo = 'svn\+ssh://svn.cern.ch/reps/tdr2/papers/'; #this only works for papers currently
   my $svn_tag = '';
   my $svn_year = 1900;
   ...
   if (m/$pat1/s)
   {
       ...
       my $pat2 = "\\\\RCS\\\$"."HeadURL:\\s+".$repo."(\\S*)/(\\S*)/$svn_filename\\.tex\\s+"."\\\$";
       if (m/$pat2/s)
       {
           $svn_tag = $2;
       }
...
#      my $collection = XML::Twig::Elt->new(collection=>{xmlns=>"http://www.loc.gov/MARC21/slim"});
      ...
      &insert_datafield("037",$record,"CMS-".$noteCode."-".$svn_tag);
